...
def calibrate_SGs(sample_rate, samples_to_read):
  with nidaqmx.Task() as task:
    ...
    task.ai_channels.add_ai_bridge_chan("cDAQ1Mod8/ai4", bridge_config=nidaqmx.constants.BridgeConfiguration.QUARTER_BRIDGE, voltage_excit_val=3.3, nominal_bridge_resistance=351.2) #CommSG1 read as ai bridge
    task.ai_channels.add_ai_bridge_chan("cDAQ1Mod8/ai6", bridge_config=nidaqmx.constants.BridgeConfiguration.QUARTER_BRIDGE, voltage_excit_val=3.3, nominal_bridge_resistance=351.2) #CommSG2 read as ai bridge
    ...
    calib_samples = np.zeros((14, samples_to_read))
    ...
    sgmean[8:12] *= 3.3 #Multiply the ai values with excitation voltage to obtain initial voltage values.
    print ("SG initial voltages are (V): ", end="")
    print (sgmean)
    print ("Initial temperature at rod is: {}".format(sgmean[12]))
    print ("Initial temperature at wing is: {}".format(sgmean[13]))
