...
for v in vel:
  ...
  trainData = np.load('/Volumes/GoogleDrive/Shared drives/WindTunnelTests-Feb2019/Sept2020_Tests/Training_Tests/train2_Sept16/train_{}ms_{}deg.npy'.format(v,aoa))
  ave_SG1.append(np.mean(-trainData[6])) #6 is SG1 at root.
  ave_lift.append(np.mean(-trainData[14])) #14 is Lift CommSG

  #Handle temperature compensation
  ref_temp = init_temps[0] if v<=12 else init_temps[1]
  #SG1 temp. comp.
...
ax1.plot(vel, ave_SG1, '-', label = "-SG1")
ax1.plot(vel, ave_SG1_comp, ':', color=ax1.lines[0].get_color(), linewidth=0.5, label="-SG1 (compensated)")
# ax1.plot(vel[-1], ave_root_SG[-1], 'r*') #Post-experiment DP
ax2.plot(vel, ave_lift, '-', label="-SG Lift")
ax2.plot(vel, ave_lift_comp, ':', color=ax2.lines[0].get_color(), linewidth=0.5, label="-SG Lift (compensated)")
# ax2.plot(vel[-1], ave_lift[-1], 'r*') #Post-experiment DP
