...
vel = [6,8,10,12,14,16,18,20]
aoas = [0,2,4,6,8,10,12,14,16,17,18,19,20,'0_2']
test_len = 1 #minutes
...
for aoa in aoas:
  ...
  #Span the velocity range
  for i in range(len(vel)):
    trainData = np.load(test_folder+'train_{}ms_{}deg_{}min.npy'.format(vel[i], aoa, test_len))
    ...
    if vel[i] in reftemp_vels:
      index = reftemp_vels.index(vel[i])
    else:
      closest_val = np.asarray(reftemp_vels)[np.asarray(reftemp_vels) < vel[i]].max() #Gets the maximum temp_reset_vel smaller than vel
  ...
  ax1.set_xlabel("Vel (m/s)", fontsize=11)
  ax1.set_ylabel("Voltage (V)", fontsize=11)
  ax2.set_title("-SG1 readings AoA = {}deg".format(aoa), fontsize=12)
  ax2.set_xlabel("Vel (m/s)", fontsize=11)
  ax2.set_ylabel("Microstrain (ue)", fontsize=11)
  ax3.set_title("-SG Lift for AoA = {}deg".format(aoa), fontsize=12)
  ax3.set_xlabel("Vel (m/s)", fontsize=11)
  ax3.set_ylabel("Microstrain (ue)", fontsize=11)


  ax1.plot(vel, sum_PZT1, '.-', linewidth=0.5, label="PZT1")
  ax2.plot(vel, ave_SG1, '.-', linewidth=0.5, label="-SG1")
  ax2.plot(vel, ave_SG1_comp, '.:', color=ax1.lines[0].get_color(), linewidth=0.5, label="-SG1 (comp.)")
  ax3.plot(vel, ave_lift, '.-', linewidth=0.5, label="SG Lift")
  ax3.plot(vel, ave_lift_comp, '.:', color=ax2.lines[0].get_color(), linewidth=0.5, label="SG Lift (comp.)")
  ...
  ax1.set_xticks(vel[::2])
  ax2.set_xticks(vel[::2])
  ax3.set_xticks(vel[::2])
  plt.tight_layout(pad=2.0)
  # plt.savefig(test_folder+'{}deg_varSpeed.png'.format(aoa))
  # plt.show()